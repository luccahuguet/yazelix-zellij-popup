...
With `hide`, pressing the toggle key while the popup is focused or replacing it with another configured popup hides that pane without killing the popup process and runs `on_hide`. Pressing the toggle key again shows and focuses the existing pane when its process cwd still matches the focused terminal cwd. If that cwd no longer matches, toggle closes the stale pane (and runs `on_close` if set) and opens a fresh popup at the current focused cwd. The explicit `close` action still closes the pane and runs `on_close`.
